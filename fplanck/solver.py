...
def expm_torch(A, B):
    A_tensor = torch.FloatTensor(A.toarray()).to(DEVICE)
    B_tensor = torch.FloatTensor(B).to(DEVICE)

    result = torch.matmul(torch.matrix_exp(A_tensor), B_tensor)
    return result.detach().to("cpu").numpy()


def expm_multiply_torch(A, B, start, stop, num, endpoint):
    ...
    init_result = torch.matmul(torch.matrix_exp(A_tensor * start), B_tensor)
    pt_list.append(init_result.detach().to("cpu").numpy())

    time_interval = float(stop - start) / num
    accumulate_number = num
    if not endpoint:
        accumulate_number -= 1

    for i in range(accumulate_number):
        if i == 0:
            continue

        tmp_pt_result = torch.matmul(torch.matrix_exp(A_tensor * (start + time_interval * i)), B_tensor)
        # tmp_pt_result = torch.matmul(A_tensor * time_interval, init_result)
        pt_list.append(tmp_pt_result.detach().to("cpu").numpy())

    return np.array(pt_list)


class fokker_planck:
    def __init__(self, *, temperature, drag, extent, resolution, center,
            potential=None, force=None, boundary=fplanck.boundary.reflecting, grid=None):
        """
        Solve the Fokker-Planck equation

        Arguments:
            temperature     temperature of the surrounding bath (scalar or vector)
            drag            drag coefficient (scalar or vector or function), drag = 1/mobility
        ...
        if grid is None:
            ...
            for i in range(len(axes)):
                axes[i] += (center[i] - np.average(axes[i]))
            self.grid = np.array(np.meshgrid(*axes, indexing='ij'))
        else:
            self.Ngrid = np.array([len(grid[0]), len(grid[0][0])])
            self.grid = grid
    ...
    def propagate(self, initial, time, normalize=True, dense=False):
        ...
        if normalize:
            p0_volume = np.trapz(np.trapz(p0, self.grid[0][:, 0], axis=0), self.grid[1][0, :], axis=0)
            p0 /= p0_volume

        if dense:
            # pf = expm(self.master_matrix*time) @ p0.flatten()
            pf = expm_torch(self.master_matrix * time, p0.flatten())
        else:
    ...
    def propagate_interval(self, initial, tf, Nsteps=None, dt=None, normalize=True):
        ...
        pf = expm_multiply_torch(self.master_matrix, p0.flatten(), start=0, stop=tf, num=Nsteps, endpoint=True)
